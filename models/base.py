...
class BaseLearner(object):
        ...
        for epoch in range(run_epochs):
            ...
            for _iter in range(crct_num):
                ...
                if self.logit_norm is not None:
                    ...
                    # loss = loss_cos(decoupled_logits, tgt)
                else:
                    loss = F.cross_entropy(logits[:, :crct_num], tgt)
                    # loss = loss_cos(logits[:, :crct_num], tgt)
    ...
    def _compute_accuracy(self, model, loader):
        # model.convnet.get_mean_adapter()
        model.eval()
        correct, total = 0, 0
    ...
    def _eval_cnn(self, loader):
        # self._network.convnet.get_mean_adapter()
        self._network.eval()
        y_pred, y_true = [], []
    ...
    def _extract_vectors_adv(self, loader, old=False):
        ...
        return np.concatenate(vectors), np.concatenate(targets)
        # return torch.from_numpy(np.concatenate(vectors)), torch.from_numpy(np.concatenate(targets))
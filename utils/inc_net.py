...
def get_convnet(args, pretrained=False):
    ...
    if name == "pretrained_vit_b16_224" or name == "vit_base_patch16_224":
        ...
        if args["model_name"] == "adapter":
            from network import vision_transformer_adapter3
            ...
            if name == "pretrained_vit_b16_224_adapter":
                model = vision_transformer_adapter3.vit_base_patch16_224_adapter(num_classes=0,
                                                                                global_pool=False, drop_path_rate=0.0,
                                                                                tuning_config=tuning_config)
                model.out_dim = 768
            elif name == "pretrained_vit_b16_224_in21k_adapter":
                model = vision_transformer_adapter3.vit_base_patch16_224_in21k_adapter(num_classes=0,
                                                                                      global_pool=False,
                                                                                      drop_path_rate=0.0,
                                                                                      tuning_config=tuning_config)
                model.out_dim = 768
...
class SimpleVitNet(BaseNet):
    ...
    def extract_vector_mixed(self, x):
        x = self.convnet.post_forward(x)
        return x
    ...
    def forward_train(self, x):
        x = self.convnet.forward_train(x)
        # x = self.convnet.module.forward_train(x)
        out = self.fc(x)
        # out.update(x)
        return out

    def pre_forawrd(self, x):
        x = self.convnet.pre_forward(x)
        return x

    def post_forawrd(self, x):
        x = self.convnet.post_forward(x)
        out = self.fc(x)
        # out.update(x)
        return out

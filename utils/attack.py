...
class Attack(object):
    ...
    def run(self, args):
        p_data, p_label = [], []
        loss_cos = AngularPenaltySMLoss(loss_type='cosface', eps=1e-7, s=args["scale"], m=args["margin"])
        for data, label in self.loader:
            ...
            for i in range(self.epochs):
                ...
                # feats = self.old_model.extract_vector(data)
                # L = nn.MSELoss()
                logits = self.old_model(data)["logits"]

                # L = loss_cos(logits, target)
                # in a targeted attack, we take the loss w.r.t. the target prototype
                loss = loss_cos(logits, target)
                ...
                if i == (self.epochs - 1):
                    adv_output = self.old_model.extract_vector(perturbed_data).cpu()
                    ...
                    if success > 0:
                        ...
                        return [], []
            p_data = torch.cat(p_data, 0).detach().cpu()
            p_label = torch.cat(p_label, 0).detach().cpu()

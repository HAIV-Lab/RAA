...
class SimpleContinualLinear(nn.Module):
    ...
    def generate_dummy(self, num_classes):
        single_head = []
        _fc = nn.Linear(self.embed_dim, num_classes, bias=False)
        trunc_normal_(_fc.weight, std=.02)
        single_head.append(_fc)
        self.dummy_head = nn.Sequential(*single_head)

    def add_dummy(self):
        self.heads.append(self.dummy_head)

    def delete_dummy(self):
        # self.dummy_head = self.heads[-1]
        del(self.heads[-1])

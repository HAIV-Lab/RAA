...
import math
from utils.flatness_minima import SAM
import os
import time
...
class Learner(BaseLearner):
    ...
    def incremental_train(self, data_manager):
        ...
        if self._cur_task > 0:
            ...
            self._network.convnet.freeze_old_adapter()
        ...
        self._network.convnet.merge_adapter()
        ...
        if self._cur_task == 0:
            ...
            if self.args['optimizer'] == 'sgd':
                ...
                if self.args["perturb"]:
                    optimizer = SAM(optimizer, self._network, rho=self.args["rho"])
            ...
            if not self.args["perturb"]:
                ...
                scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer.optimizer, T_max=self.tuned_epochs,
                                                                 eta_min=self.min_lr)
        ...
        if not self.args["perturb"]:
            for _, epoch in enumerate(prog_bar):
                ...
                for i, (_, inputs, targets) in enumerate(train_loader):
                    batch = inputs.size(0)
                    inputs = [inputs for _ in range(self.args['adapter_num'] + 1)]
                    targets = [targets for _ in range(self.args['adapter_num'] + 1)]
                    inputs = torch.cat(inputs, dim=0)
                    targets = torch.cat(targets, dim=0)
                    inputs, targets = inputs.to(self._device), targets.to(self._device)

                    # self._network.convnet.set_fused_adapter()

                    logits = self._network.forward_train(inputs)["logits"]
                    logiits_1 = logits[:-batch, :]
                    targets_1 = targets[:-batch]
                    logiits_2 = logits[-batch:, :]
                    targets_2 = targets[-batch:]
                    loss = self.args["loss_weight"] * loss_cos(logiits_1[:, self._known_classes:],
                                                               targets_1 - self._known_classes) + loss_cos(
                        logiits_2[:, self._known_classes:], targets_2 - self._known_classes)
            ...
            for _, epoch in enumerate(prog_bar):
                ...
                for i, (_, inputs, targets) in enumerate(train_loader):
                    ...
                    logiits_2 = logits[-batch:, :]
                    targets_2 = targets[-batch:]
                    loss = self.args["loss_weight"]*loss_cos(logiits_1[:, self._known_classes:], targets_1 - self._known_classes) + loss_cos(logiits_2[:, self._known_classes:], targets_2 - self._known_classes)
                    # loss = 0.1*F.cross_entropy(logiits_1, targets_1) + F.cross_entropy(logiits_2, targets_2)
                    ...
                    # self._network.convnet.del_fused_adapter()
